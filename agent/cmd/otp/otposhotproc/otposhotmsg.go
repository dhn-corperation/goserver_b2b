...
func MSGProcess(ctx context.Context) {
	...
	for {
		select {
		    ...
		    config.Stdlog.Println("Oshot OTP MSG process가 10초 후에 종료 됨.")
		    time.Sleep(10 * time.Second)
		    config.Stdlog.Println("Oshot OTP MSG process 종료 완료")
		    return
		default:	
			var t = time.Now()
...
func msgProcess(wg *sync.WaitGroup) {
	...
	defer func(){
		if r := recover(); r != nil {
			config.Stdlog.Println("Oshot OTP msgProcess panic 발생 원인 : ", r)
			if err, ok := r.(error); ok {
				if s.Contains(err.Error(), "connection refused") {
					for {
						config.Stdlog.Println("Oshot OTP msgProcess send ping to DB")
	...
	var MMSTable = "OTP_OShotMSG_" + monthStr
	...
	if err != nil {
		...
		errcode := err.Error()
		errlog.Println("OShot OTP MSG 조회 중 오류 발생", groupQuery, errcode)

		if s.Index(errcode, "1146") > 0 {
			db.Exec("Create Table IF NOT EXISTS " + MMSTable + " like OTP_OShotMSG")
