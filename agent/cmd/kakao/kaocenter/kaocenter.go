...
import (
	...
	config "mycs/agent/configs"
	db "mycs/agent/configs/databasepool"
	cm "mycs/agent/internal/commons"
	ss "mycs/agent/internal/structs"
...
func CreateTemplateNps(c *fasthttp.RequestCtx) {
	...
	var kakakoReqParam ss.CtKakaoReq
	var kakakoResParam ss.StKakaoRes
	var kakakoResParam2 ss.StKakaoRes

	rawData := string(c.PostBody())

	config.Stdlog.Println(rawData)
	...
	if templateCode != nil {
		strTemplateCode := templateCode.(interface{})
		kakakoReqParam.TemplateCode = &strTemplateCode
	}

	if templateName != nil {
		strTemplateName := templateName.(interface{})
		kakakoReqParam.TemplateName = &strTemplateName
	}

	if templateContent != nil {
		strTemplateContent := templateContent.(interface{})
	...
	if buttons != nil {
	    ...
	    if !ok {
	        config.Stdlog.Println("Invalid buttons format, expected []interface{}, got:", fmt.Sprintf("%T", buttons))
	        kakakoReqParam.Buttons = &[]ss.KakaoButtonsNps{} // 빈 배열을 할당하여 오류를 방지
	    } else if len(btnList) > 0 {
	        var tempBts []ss.KakaoButtonsNps
	        for _, item := range btnList {
	            ...
	            var tempBt ss.KakaoButtonsNps

	            if name, exists := v["name"]; exists {
	                tempBt.Name = &name
	            }
	            if linkType, exists := v["type"]; exists {
	                tempBt.LinkType = &linkType
	            }
	            if ordering, exists := v["ordering"]; exists {
	                tempBt.Ordering = &ordering
	            }
	            if linkAnd, exists := v["schemeAndroid"]; exists {
	                tempBt.LinkAnd = &linkAnd
	            }
	            if linkIos, exists := v["schemeIos"]; exists {
	                tempBt.LinkIos = &linkIos
	            }
	            if linkMo, exists := v["url_mobile"]; exists {
	                tempBt.LinkMo = &linkMo
	            }
	            if linkPc, exists := v["url_pc"]; exists {
	                tempBt.LinkPc = &linkPc
	            }
	            if pluginId, exists := v["pluginId"]; exists {
	...
	if strings.EqualFold(*kakakoResParam.Code, "200") {
		var reqData ss.KsReqNps
		var reqRes ss.KtrResNps
...
func SearchTemplateNps(c *fasthttp.RequestCtx) {
	...
	config.Stdlog.Println(*kakakoResParam.Code)
	if *kakakoResParam.Code == "200" {
...
func UpdateTemplateNps(c *fasthttp.RequestCtx) {
	...
	var kakakoReqParam ss.UtKakaoReq
	var kakakoResParam ss.StKakaoRes
	var kakakoResParam2 ss.StKakaoRes
...
func SetComment(c *fasthttp.RequestCtx) {
	...
	if strings.EqualFold(*kakakoResParam.Code, "200") {
		if strings.EqualFold(kakakoResParam.Data.InspectionStatus, "REG") {
			...
			tc := interface{}(*kakakoResParam.Data.TemplateCode)
			kakakoReqParam.TemplateCode = &tc
			...
			ntc := interface{}(*kakakoResParam.Data.TemplateCode)
			kakakoReqParam.NewTemplateCode = &ntc
			tn := interface{}(*kakakoResParam.Data.TemplateName)
			kakakoReqParam.NewTemplateName = &tn
			tct := interface{}(*kakakoResParam.Data.TemplateContent)
			kakakoReqParam.NewTemplateContent = &tct
			kakakoReqParam.NewTemplateMessageType = "BA"
			kakakoReqParam.NewTemplateEmphasizeType = "NONE"
...
func CancelApproveTemplate(c *fasthttp.RequestCtx){
	...
	var kakakoReqParam ss.UtKakaoReq
	var kakakoResParam ss.KtrResNps
...
// 템플릿 검수 함수
func templateRequestNps(data ss.KsReqNps) ss.KtrResNps {
...
// 템플릿 조회 함수
func templateNps(data ss.KsReqNps) ss.StKakaoRes {
	config.Stdlog.Println("templateNps 시작")
	var reqRes ss.StKakaoRes
...
// 템플릿 검수 취소 함수
func cancelTemplateRequestNps(data ss.KsReqNps) ss.KtrResNps {
...
// 템플릿 승인 취소 함수
func cancelApproveTemplateNps(data ss.UtKakaoReq) ss.KtrResNps {
