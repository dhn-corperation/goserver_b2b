...
import(
	...
	databasepool "mycs/agent/configs/databasepool"
	cm "mycs/agent/internal/commons"
	krt "mycs/agent/cmd/kakao/kaoresulttable"
)
	...
	for {
		if procCnt < 3 {
			...
			default:
				...
				if err != nil {
					config.Stdlog.Println("Friendtalk 9999 resend 트랜잭션 초기화 실패 : ", err)
					continue
				}
				...
				if err != nil {
					config.Stdlog.Println("Friendtalk 9999 resend RowsAffected 확인 오류 : ", err)
					tx.Rollback()
					continue
				}
				...
				if err := tx.Commit(); err != nil {
					config.Stdlog.Println("Friendtalk 9999 resend tx Commit 오류 : ", err)
					tx.Rollback()
					continue
				}

				procCnt++
				config.Stdlog.Println("Friendtalk 9999 resend 발송 처리 시작 ( ", group_no, " ) : ", rowCount, " 건  ( Proc Cnt :", procCnt, ") - START")
...
func ftResendProcess(group_no string, pc int) {
	...
	for reqrows.Next() {
		...
		var friendtalk ss.Friendtalk
		var attache ss.Attachment
		var tcarousel ss.TCarousel
		var carousel ss.FCarousel
		var button []ss.Button
		var image ss.Image
		var coupon ss.AttCoupon
		var itemList ss.AttItem
			...
			case "carousel":
				if z, ok := (scanArgs[i]).(*sql.NullString); ok {
					...
					for ci, _ := range tcarousel.List {
						var catt ss.CarouselAttachment
						var tcarlist ss.CarouselList
