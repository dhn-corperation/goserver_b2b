...
func LMSProcess(ctx context.Context) {
	...
	OshotTables, err := db.Query(oshotQuery)
	...
	for OshotTables.Next() {
		OshotTables.Scan(&otable)
	...
	for {
			...
			default:	
				...
				for _, tableName := range oshotTable {
					...
					if t.Day() < 3 {
						wg.Add(1)
						go mmsProcess(&wg, tableName[0], true)
					}
		
					wg.Add(1)
					go mmsProcess(&wg, tableName[0], false)
				}
				wg.Wait()
			}
	}
...
func mmsProcess(wg *sync.WaitGroup, ostable string, nsFlag bool) {
	...
	var groupQuery = "select cb_msg_id, SendResult, SendDT, MsgID, telecom, userid  from " + MMSTable + " a where a.proc_flag = 'Y' "
