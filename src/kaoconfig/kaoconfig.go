...
func createConfig(dirName string) error {
	...
	configData := []string{
		...
		`# 1) 알림톡, 친구톡 1회 발송 묶음 건수`,
		`KAKAO_SENDLIMIT = 500`,
		`# 2) 알림톡 전송 방식 : push, polling`,
		`RESPONSE_METHOD = "push"`,
		`# 3) 2차 발송 플래그(1차 발송이 알림톡, 친구톡 일 시)`,
		`PHONE_MSG_FLAG = "YES"`,
		`CHANNEL = "채널명"`,
		`# 4) 친구톡 데이터 확인 플래그`,
		`DEBUG = "NO"`,
		...
		``,
		`# 5. 나노 메시지 관련`,
		`# 1) default identification code`,
		`NANO_IDENTI_CODE = "302190001"`,
		`# 2) 전화번호 별 분리(YES 일 시 010과 010 아닌 것의 분리, NO 일 시 구분없이 전송)`,
		`NANO_TYPE_FLAG = "N"`,
		``,
		`#추가할 설정 내용 필요에 따라 작성`,
	}
