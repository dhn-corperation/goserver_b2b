...
func InitConfig() {
	path := "/root/DHNServer_callbs/log/DHNServer"
	//path := "./log/DHNServer"
	...
	Conf = readConfig()
	BasePath = "/root/DHNServer_callbs/"
	Client = resty.New()

}

func readConfig() Config {
	var configfile = "/root/DHNServer_callbs/config.ini"
...
func InitCenterConfig() {
	path := "/root/DHNCenter_callbs/log/DHNCenter"
	//	path := "./log/DHNCenter"
	loc, _ := time.LoadLocation("Asia/Seoul")
	writer, err := rotatelogs.New(
		fmt.Sprintf("%s-%s.log", path, "%Y-%m-%d"),
	...
	BasePath = "/root/DHNCenter_callbs/"
	//Client = resty.New()

}

func readCenterConfig() Config {
	var configfile = "/root/DHNCenter_callbs/config.ini"
	//	var configfile = "./config.ini"
	_, err := os.Stat(configfile)
	if err != nil {
		fmt.Println("Config file is missing : ", configfile)
