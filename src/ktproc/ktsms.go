...
func smsProcess(wg *sync.WaitGroup, table string, preFlag bool, seq int, acc int) {
	...
	if isProc {
		...
		for searchData.Next() {
			...
			_, err = db.Exec(`insert into ` + SMSTable + `(userid, msgid, MessageSubType, CallbackNumber, Bundle_Seq, Bundle_Num, Bundle_Content, Bundle_Subject, Image_path1, Image_path2, Image_path3, resp_JobID, resp_Time, resp_SubmitTime, resp_Result, Resp_TelconInfo, resp_EndUserID, resp_ServiceProviderID, sep_seq, dhn_id)
