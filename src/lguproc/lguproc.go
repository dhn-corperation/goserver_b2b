...
func resProcess(ctx context.Context, group_no string, user_id string) {
	...
	defer func() {
		if err := recover(); err != nil {
			procCnt--
			stdlog.Println(user_id, "- ", group_no, "Lgu 처리 중 오류 발생 : ", err)
		}
	}()
	...
	for resrows.Next() {
		...
		if len(osmmsStrs) > 500 {
			stmt := fmt.Sprintf("insert into LG_MMS_MSG(SUBJECT, PHONE, CALLBACK, REQDATE, MSG, FILE_CNT, FILE_PATH1, FILE_PATH2, FILE_PATH3, ETC1, ETC2, ETC3, KISA_ORIGCODE) values %s", s.Join(osmmsStrs, ","))
		...
		if len(msg_sms.String) > 0 && len(sms_sender.String) > 0 { // msg_sms 가 와 sms_sender 에 값이 있으면 Oshot 발송 함.
			...
			if s.EqualFold(sms_kind.String, "S") {
				if msgLen.Int64 <= 90 || s.EqualFold(sms_len_check.String, "N") {
					...
					ossmsValues = append(ossmsValues, "302190001")
					smscnt++
				} else {
				...
				osmmsValues = append(osmmsValues, "302190001")
				lmscnt++
			}
