...
import (
	...
	db "goserver/src/kaodatabasepool"
	"io"
	"net"
	"net/http"
	"path/filepath"
...
func Sender_token(c *gin.Context) {
	...
	req, err := http.NewRequest("GET", conf.CENTER_SERVER+"api/v1/"+conf.PROFILE_KEY+"/sender/token?"+query.Encode(), nil)
	if err != nil {
...
func Category_(c *gin.Context) {
	...
	req, err := http.NewRequest("GET", conf.CENTER_SERVER+"api/v1/"+conf.PROFILE_KEY+"/category?"+query.Encode(), nil)
	if err != nil {
		c.JSON(http.StatusBadRequest, err.Error())
		return
	}
...
func Sender_Create(c *gin.Context) {
	...
	req.Header.Add("phoneNumber", params["phoneNumber"])
...
func Sender_(c *gin.Context) {
	...
	req, err := http.NewRequest("GET", conf.CENTER_SERVER+"api/v3/"+conf.PROFILE_KEY+"/sender?"+query.Encode(), nil)
	if err != nil {
		c.JSON(http.StatusBadRequest, err.Error())
		return
	}
...
func Template_Create_Image(c *gin.Context) {
	...
	if err != nil {
		config.Stdlog.Println("File Copy 오류 : ", err)
	}
...
func Template_(c *gin.Context) {
	...
	params := map[string]string{
		"senderKey":     c.Query("senderKey"),
		"templateCode":  c.Query("templateCode"),
		"senderKeyType": c.DefaultQuery("senderKeyType", "S"),
	}
	...
	//req, err := http.NewRequest("GET", conf.CENTER_SERVER+"api/v2/"+conf.PROFILE_KEY+"/alimtalk/template?senderKey="+params["senderKey"]+"&templateCode="+url.QueryEscape(params["templateCode"])+"&senderKeyType="+params["senderKeyType"], nil)
	req, err := http.NewRequest("GET", conf.CENTER_SERVER+"api/v2/"+conf.PROFILE_KEY+"/alimtalk/template?"+query.Encode(), nil)
	if err != nil {
		c.JSON(http.StatusBadRequest, err.Error())
		return
	}
...
func Template_Last_Modified(c *gin.Context) {
	...
	req, err := http.NewRequest("GET", conf.CENTER_SERVER+"api/v3/"+conf.PROFILE_KEY+"/alimtalk/template/last_modified?"+query.Encode(), nil)
	if err != nil {
		c.JSON(http.StatusBadRequest, err.Error())
		return
	}
...
func Template_Category_(c *gin.Context) {
	...
	req, err := http.NewRequest("GET", conf.CENTER_SERVER+"api/v2/"+conf.PROFILE_KEY+"/alimtalk/template/category?"+query.Encode(), nil)
	if err != nil {
		c.JSON(http.StatusBadRequest, err.Error())
		return
	}
...
func Group_Sender(c *gin.Context) {
	...
	req, err := http.NewRequest("GET", conf.CENTER_SERVER+"api/v3/"+conf.PROFILE_KEY+"/group/sender?"+query.Encode(), nil)
	if err != nil {
		c.JSON(http.StatusBadRequest, err.Error())
		return
	}
...
func Channel_(c *gin.Context) {
	...
	req, err := http.NewRequest("GET", conf.CENTER_SERVER+"api/v2/"+conf.PROFILE_KEY+"/channel?"+query.Encode(), nil)
	if err != nil {
		c.JSON(http.StatusBadRequest, err.Error())
		return
	}
...
func Plugin_CallbackUrls_List(c *gin.Context) {
	...
	req, err := http.NewRequest("GET", conf.CENTER_SERVER+"api/v1/"+conf.PROFILE_KEY+"/plugin/callbackUrl/list?"+query.Encode(), nil)
	if err != nil {
		c.JSON(http.StatusBadRequest, err.Error())
		return
	}
...
func Image_wideItemList(c *gin.Context) {
	conf := config.Conf
	param, err := image_Seq_Mapping(c, map[string]io.Reader{}, 4, "image")
	if err != nil {
		config.Stdlog.Println("image Mapping 오류 : ", err)
	}

	resp, _ := upload(conf.IMAGE_SERVER+"v1/"+conf.PROFILE_KEY+"/image/friendtalk/wideItemList", param)
	bytes, _ := io.ReadAll(resp.Body)
	c.Data(http.StatusOK, "application/json", bytes)
}
...
func Image_carousel(c *gin.Context) {
	conf := config.Conf
	param, err := image_Seq_Mapping(c, map[string]io.Reader{}, 6, "image")
	if err != nil {
		config.Stdlog.Println("image Mapping 오류 : ", err)
	}
