			...
			default:
				...
				if cnterr != nil {
					...
					if count > 0 {
						...
						updateRows, err := databasepool.DB.Exec("update DHN_REQUEST set send_group = '" + group_no + "' where send_group is null and (reserve_dt IS NULL OR to_timestamp(coalesce(reserve_dt,'00000000000000'), 'YYYYMMDDHH24MISS') <= NOW()) limit " + strconv.Itoa(config.Conf.SENDLIMIT))
