	...
	for {
		if atprocCnt <=5 {
			...
			default:
				...
				cnterr := databasepool.DB.QueryRowContext(ctx, "SELECT LENGTH(msgid) AS cnt FROM DHN_REQUEST_AT WHERE send_group IS NULL AND coalesce(reserve_dt::timestamp,'00000000000000')::timestamp <= NOW() AND userid=?", user_id).Scan(&count)
