...
func atsendProcess(group_no string, user_id string) {
	...
	for reqrows.Next() {
		...
		for i, v := range columnTypes {
			...
			if z, ok := (scanArgs[i]).(*sql.NullString); ok {
				result[s.ToLower(v.Name())] = cleanNull(z.String)
			}

			if z, ok := (scanArgs[i]).(*sql.NullInt32); ok {
				result[s.ToLower(v.Name())] = cleanNull(string(z.Int32))
			}

			if z, ok := (scanArgs[i]).(*sql.NullInt64); ok {
				result[s.ToLower(v.Name())] = cleanNull(string(z.Int64))
			}
...
func cleanNull(in string) string {
	return s.Replace(in, "\x00", "", -1)
}
