	...
	for {
		if atprocCnt <=5 {
			...
			default:
				...
				cnterr := databasepool.DB.QueryRowContext(ctx, "SELECT LENGTH(msgid) AS cnt FROM DHN_REQUEST_AT WHERE send_group IS NULL AND CAST(coalesce(reserve_dt,'00000000000000') AS INTEGER) <= CAST(TO_CHAR(now(), '%Y%m%d%H%i%S') AS INTEGER) AND userid=?", user_id).Scan(&count)
				...
				if cnterr != nil && cnterr != sql.ErrNoRows {
					...
					if count.Valid && count.Int64 > 0 {		
						...
						updateRows, err := databasepool.DB.ExecContext(ctx, "update DHN_REQUEST_AT set send_group = ? where send_group is null and coalesce(reserve_dt,'00000000000000') <= TO_CHAR(now(), '%Y%m%d%H%i%S') and userid = ?  limit ?", group_no, user_id, strconv.Itoa(config.Conf.SENDLIMIT))
