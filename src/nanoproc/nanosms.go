...
func NanoSMSProcess(ctx context.Context) {
	...
	for {
		select {
		    ...
		    config.Stdlog.Println("Nano SMS process가 10초 후에 종료 됨.")
...
func smsProcess(wg *sync.WaitGroup) {
	...
	defer func(){
		if r := recover(); r != nil {
			config.Stdlog.Println("NANO smsProcess panic 발생 원인 : ", r)
			if err, ok := r.(error); ok {
				if s.Contains(err.Error(), "connection refused") {
					for {
						config.Stdlog.Println("NANO smsProcess send ping to DB")
