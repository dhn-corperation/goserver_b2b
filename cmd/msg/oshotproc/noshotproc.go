...
func nresProcess(ctx context.Context, group_no, user_id string, pc int) {
	...
	SELECT
		...
		res_dt, 
		reserve_dt, 
		(select file1_path from API_MMS_IMAGES aa where aa.userid = drr.userid and aa.mms_image_id = drr.mms_image_id) as mms_file1, 
		(select file2_path from API_MMS_IMAGES aa where aa.userid = drr.userid and aa.mms_image_id = drr.mms_image_id) as mms_file2, 
		(select file3_path from API_MMS_IMAGES aa where aa.userid = drr.userid and aa.mms_image_id = drr.mms_image_id) as mms_file3
